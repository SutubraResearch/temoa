...
def demand_constraint_error_check(
    supply: Any, r: Region, p: Period, s: Season, d: TimeOfDay, dem: Commodity
) -> None:
    # note:  if a pyomo equation simplifies to an int, there are no variables in it, which
...
def demand_constraint_indices(
    model: TemoaModel,
) -> set[tuple[Region, Period, Season, TimeOfDay, Commodity]]:
    """Index set for timeslice-level demand constraints."""
    return {
        (r, p, s, d, dem)
...
def demand_constraint(
    model: TemoaModel, r: Region, p: Period, s: Season, d: TimeOfDay, dem: Commodity
) -> ExprLike:
    r"""
    ...
    Demand techs use timeslice-level :math:`\textbf{FO}` variables only (not annual).
    True :code:`tech_annual` techs (if any serve demand) use :math:`\textbf{FOA}`
    scaled by segment fraction.
    ...
    supply = sum(
        model.v_flow_out[r, p, s, d, s_i, s_t, s_v, dem]
    ...
    demand_constraint_error_check(supply + supply_annual, r, p, s, d, dem)

    expr = supply + supply_annual == (
        value(model.demand[r, p, dem]) * value(model.demand_specific_distribution[r, p, s, d, dem])
    )
