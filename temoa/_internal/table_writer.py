...
class TableWriter:
    ...
    def _get_metadata_real(self, element: str) -> float | None:
        """Fetch a metadata_real value if available, otherwise return None."""
        try:
            query = 'SELECT value FROM metadata_real WHERE element = ?'
            row = self.connection.execute(query, (element,)).fetchone()
    ...
    def _resolve_output_threshold(self, threshold_type: str) -> float:
        ...
        db_value = self._validate_threshold(toml_key, self._get_metadata_real(toml_key))
